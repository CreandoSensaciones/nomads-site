...
class CurrencyForm extends EntityForm {
  ...
  public function form(array $form, FormStateInterface $form_state) {
    ...
    $form['currency_code'] = [
      ...
      '#element_validate' => [[$this, 'validateCurrencyCode']],
      '#maxlength' => 3,
      '#pattern' => '[a-zA-Z]{3}',
      '#placeholder' => 'XXX',
      '#required' => TRUE,
    ...
    $form['currency_number'] = [
      '#default_value' => $currency->getCurrencyNumber(),
      '#element_validate' => [[$this, 'validateCurrencyNumber']],
      '#maxlength' => 3,
      '#pattern' => '[\d]{3}',
      '#placeholder' => '999',
      '#size' => 3,
    ...
    $form['rounding_step'] = [
      ...
      '#element_validate' => [[$this, 'validateRoundingStep']],
      '#required' => TRUE,
  ...
  public function save(array $form, FormStateInterface $form_state): int {
    ...
    $this->messenger()->addMessage($this->t('The currency %label has been saved.', [
  ...
  public function validateCurrencyCode(array $element, FormStateInterface $form_state, array $form) {
    ...
    elseif ($currency->isNew()) {
      ...
      if ($loaded_currency) {
        $form_state->setError($element, $this->t('The currency code is already in use by @link.', [
  ...
  public function validateCurrencyNumber(array $element, FormStateInterface $form_state, array $form) {
    ...
    elseif ($currency->isNew()) {
      $loaded_currencies = $this->currencyStorage->loadByProperties([
        'currencyNumber' => $currency_number,
      ]);
      if ($loaded_currencies) {
        $loaded_currency = reset($loaded_currencies);
        $form_state->setError($element, $this->t('The currency number is already in use by @link.', [
          '@link' => $this->linkGenerator->generate($loaded_currency->label(), $loaded_currency->toUrl('edit-form')),
        ]));
      }
    }
