...
/**
 * Tests the currency user interface.
 *
 * @group Currency
 */
class CurrencyWebTest extends BrowserTestBase {
  ...
  public function testUserInterface() {
    ...
    // Overview pages without actionable items should be inaccessible.
    $this->assertSession()->statusCodeEquals('403');
    $this->drupalGet($currency_overview_path);
    $this->assertSession()->statusCodeEquals('403');
    $account = $this->drupalCreateUser(['currency.currency.view', 'access administration pages']);
    $this->drupalLogin($account);
    $this->drupalGet($regional_path);
    ...
    $this->drupalGet($currency_overview_path);
    $this->assertSession()->statusCodeEquals('403');
    $account = $this->drupalCreateUser(['currency.currency.view']);
    $this->drupalLogin($account);
    $this->drupalGet($currency_overview_path);
    $this->assertSession()->statusCodeEquals('200');
