 ...
 * - apply non-matching domain restriction.
 */
class DomainMenuLinkTreeManipulators {
  ...
  protected function menuLinkCheckAccess(MenuLinkInterface $instance, LanguageInterface $current_language) {
    ...
    if ($entity = $this->loadMenuLinkContentEntity($instance)) {
      ...
      if (!$this->isAvailableOnAllAffiliates($entity)) {
        $domain_access = [];
        foreach ($entity->get(DomainAccessManagerInterface::DOMAIN_ACCESS_FIELD)->getValue() as $reference) {
          $domain_access[] = $reference['target_id'];
        }

        $active_domain = $this->domainNegotiator->getActiveDomain();
        if (($active_domain instanceof DomainInterface) && !in_array($active_domain->getOriginalId(), $domain_access, TRUE)) {
   ...
   *
   * @return bool
   *   Returning status of availability on all affiliates.
   */
  protected function isAvailableOnAllAffiliates(MenuLinkContent $entity) {
    if ($entity->get(DomainAccessManagerInterface::DOMAIN_ACCESS_ALL_FIELD)->isEmpty()) {
      return FALSE;
    }

    $all_affiliates = $entity->get(DomainAccessManagerInterface::DOMAIN_ACCESS_ALL_FIELD)
      ->first()
      ->getString();
  ...
  protected function loadMenuLinkContentEntity(MenuLinkInterface $instance) {
    ...
    if (isset($instance->getPluginDefinition()['metadata']['entity_id'])) {
