...
// WICHTIGE ÄNDERUNG FÜR DRUPAL 11/12:
// Ersetzt die veraltete Klasse FormElement durch FormElementBase.
use Drupal\Core\Render\Element\FormElementBase;
use Drupal\Core\Form\FormStateInterface;
use Drupal\Component\Utility\Number as NumberUtility;
use Drupal\Component\Utility\NestedArray;
use Drupal\Core\StringTranslation\TranslatableMarkup as SafeTranslation;
 ...
 * @FormElement("slider")
 */
// HAUPTÄNDERUNG: Erweitert FormElementBase statt der veralteten FormElement.
class Slider extends FormElementBase {
  ...
  public static function validateSlider(&$element, FormStateInterface $form_state, &$complete_form) {
    ...
    foreach ($values as $value) {
      ...
      if (!is_numeric($value)) {
        $form_state->setError($element, new SafeTranslation('%name must be a number.', ['%name' => $name]));
        return;
      }

      // Ensure that the input is greater than the #min property, if set.
      if (isset($element['#min']) && $value < $element['#min']) {
        $form_state->setError($element, new SafeTranslation('%name must be higher than or equal to %min.', [
          '%name' => $name,
          '%min' => $element['#min'],
        ]));
      }
      ...
      if (isset($element['#max']) && $value > $element['#max']) {
        $form_state->setError($element, new SafeTranslation('%name must be lower than or equal to %max.', [
          '%name' => $name,
          '%max' => $element['#max'],
        ]));
      }
      ...
      if (isset($element['#step']) && strtolower($element['#step']) != 'any') {
        ...
        if (!NumberUtility::validStep($value, $element['#step'], $offset)) {
          $form_state->setError($element, new SafeTranslation('%name is not a valid number.', ['%name' => $name]));
        }
      }
    }
  }
  ...
  public static function preRenderSlider(array $element) {
    ...
    if (isset($element['#attributes']) && is_array($element['#attributes'])) {
      $array = [$_attributes_new, $element['#attributes']];
      $_attributes_new = NestedArray::mergeDeepArray($array, TRUE);
    }
    ...
    if (isset($element['#ajax'])) {
      ...
      // Generate input for slider.
      // Hinweis: Die ursprüngliche Logik behält diese Struktur bei.
      if (!empty($element['slider']['value'])) {
          $element['slider']['value']['#ajax'] = $ajax;
      }
      if (!empty($element['slider']['from'])) {
          $element['slider']['from']['#ajax'] = $ajax;
      }
    }

    if (!empty($element['#display_values']) && !empty($element['#values'])) {
    ...
    if ($element['#hide_slider_handle_when_no_value']) {
      $element['slider']['note'] = [
        '#type' => 'markup',
        '#markup' => '<div class="sliderwidget-selectvalue-description">' . new SafeTranslation('Please click on any part of the slider to select a value') . '</div>',
      ];
    }
      ...
      '#attached' => [
        ...
        'drupalSettings' => [
          'sliderwidget_' . $element['#id'] => [
            ...
            // ENTFERNT: Die folgenden zwei Eigenschaften waren in getInfo()
            // nicht definiert und wurden hier entfernt, um Fehler zu vermeiden.
            ...
            'values' => !empty($element['#values']) ? $element['#values'] : [],
            'group' => $element['#group'],
    ...
    // Attach a "range" field widget library if "range" module enabled.
    if (\Drupal::moduleHandler()->moduleExists('range')) {
      // FEHLERBEHEBUNG: Fügt die Bibliothek zum Array hinzu, statt es zu überschreiben.
      $element['slider']['container']['#attached']['library'][] = 'range/range.field-widget';
    }
  ...
  public function sliderValidatePositiveNumber(array $element, FormStateInterface $form_state) {
    ...
    if (!is_numeric($value) && !is_float($value) && !empty($value)) {
      $form_state->setError($element, new SafeTranslation('The value should be a valid number'));
    }
    elseif ($value < 0) {
      $form_state->setError($element, new SafeTranslation('The value should be a valid positive number'));
    }
  }
