...
final class ListingDomainMatcher {
  ...
  private function getDomainFilterTids(DomainInterface $domain): array {
    $settings = (array) ($this->configStorage->read('nomads_navigation.settings') ?? []);
    foreach ($this->getDomainSettingsConfigNames($domain) as $config_name) {
      $this->cacheTags[] = 'config:' . $config_name;
      $domain_settings = $this->configStorage->read($config_name);
      if (is_array($domain_settings)) {
        $settings = array_replace_recursive($settings, $domain_settings);
      }
    }
  ...
  private function getDomainSettingsConfigNames(DomainInterface $domain): array {
    ...
    return [
      ...
      DomainConfigOverrider::getConfigNameByDomainAndLanguage('nomads_navigation.settings', $domain_id, $this->languageManager->getCurrentLanguage()->getId()),
    ];
  }
